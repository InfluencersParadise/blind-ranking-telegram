...
export async function POST(request: NextRequest) {
  ...
  const text = [
    `🏆 <b>Blind Ranking von ${escapeHtml(player)}</b>`,
    "",
    ...ranking.map((entry) => `${entry.position}️⃣ ${escapeHtml(entry.title)}`)
  ].join("\n");
  ...
  if (!telegramData.ok) {
    return NextResponse.json({ error: telegramData.description || "Telegram konnte nicht senden." }, { status: 502 });
  }
